...
struct Grid
{
  ...
  std::vector<Location*> getNeighbors(const Location& location)
  {
...
int main(int /*argc*/, char** argv)
{
  ...
  std::vector<Location> gridVector(width * height * 25);
  for (int j = 0; const auto& line : lines)
  {
    for (int i = 0; const auto& c : line)
    {
      gridVector[j * width * 5 + i] = Location{i, j, c - '0'};
      ++i;
    }
    ++j;
  }
  Grid g{std::move(gridVector), width, height, 5};

  auto cameFrom = aStar(g, g.get(0, 0), g.get(width * 5 - 1, height * 5 - 1));
  auto path = reconstructPath(
      g.get(0, 0), g.get(width * 5 - 1, height * 5 - 1), cameFrom);
