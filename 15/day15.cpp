...
struct Location {
  Location() = default;
  Location(int x, int y, int risk) : x(x), y(y), risk(risk) {}
  int x;
  int y;
  int risk;
...
struct Grid {
  Grid(std::vector<Location> &&grid, int o_w, int o_h, int factor)
      : grid{grid}, o_w{o_w}, o_h{o_h}, factor{factor} {}
...
std::unordered_map<Location *, Location *> aStar(Grid &g, Location &start,
                                                 Location &end) {
  std::unordered_map<Location *, Location *> cameFrom;
  std::unordered_map<Location *, int> costSoFar;
  std::priority_queue<LocationPriority, std::vector<LocationPriority>,
                      decltype(compare)>
      frontier(compare);
...
std::vector<Location *>
reconstructPath(Location &start, Location &goal,
                std::unordered_map<Location *, Location *> cameFrom) {
